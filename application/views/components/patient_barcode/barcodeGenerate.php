...
<div class="container-fluid" ng-controller="PatientBarcodeCtrl" ng-cloak>
    <div class="row">
        				...
        				<td>
        				    ...
        				    <datalist id="allProductCodes">
        				        <?php  if($allProducts != NULL){ foreach($allProducts as $key=>$value) { ?>
                                 <option value="<?php echo $value->pid; ?>"><?php echo filter($value->pid).'-'.$value->name.'-'.$value->contact; ?></option>
                                <?php } } ?>
                            </datalist>
        				</td>
        				<td>
