                    ...
                    <div class="row">
                        <div class="col-sm-6 search-area">
                            <div class="header d_flex_b">
                                <h3>All Group <small>(Select Group)</small></h3>
                                <input type="search" class="search" placeholder="Search Your Content...">
                            </div>

                            <div class="testbox">
                                <?php
                                $groups = $this->action->readGroupBy('test_group', 'id', ['trash'=>0], 'position', 'ASC');
                                foreach($groups as $group){ ?>
                                    <label class="box">
                                        <span class="checkbox">
                                            <input class="checkit allgroup" type="radio" name="group_id" value="<?=($group->id)?>">
                                        </span>
                                        <span class="finding"><?=($group->group_name)?></span>
                                    </label>
                                <?php } ?>
                            </div>
                        </div>
                        ...
                        <div class="col-sm-6 search-area">
                            ...
                            <div class="testbox">
                                <?php
                                $tests = $this->action->readGroupBy('test', 'id', ['trash'=>0], 'position', 'ASC');
                                foreach ($tests as $key => $test) { ?>
                                    <label class="box">
                                        <span class="checkbox">
                                            <input class="checkit" type="checkbox" name="test_id[]" value="<?=($test->id)?>">
                                        </span>
...
<script>
    ...
    mapping.groupMapping("<?=(site_url('alltest/group_mapping/getGroupMapping'))?>");
